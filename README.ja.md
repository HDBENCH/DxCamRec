...
映像は再圧縮しないので、画質が落ちず、CPU もほとんど使いません。
...
- **カメラの検索**。同じネットワークの ONVIF カメラを探して、映像の URL を入れてくれます。
- **タスクトレイに常駐**します。× ボタンは画面を隠すだけで録画は続き、録画中はアイコンの
  ランプが赤くなります。
- **日本語と英語**に対応し、Windows の表示言語に合わせて自動で切り替わります。
- FFmpeg も追加のランタイムも要りません。RTSP を自前で話す実行ファイル 1 つです。
...
| ファイル | 中身 |
|----------|------|
| `DxCamRec-<版>.msi` | インストーラー。Program Files に入れます。デスクトップのショートカットと自動起動を選べます。 |
| `DxCamRec-<版>-manual.zip` | 実行ファイルと説明書だけ。書き込めるフォルダに展開して実行します。 |
...
設定とログは、zip 版では実行ファイルの横、MSI で入れた場合は `C:\ProgramData\DxCamRec\` に置かれます。
